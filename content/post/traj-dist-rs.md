...
`traj-dist-rs`针对动态规划算法还做了一个内存上的优化：对于大部分动态规划算法，当我们不需要回溯动态规划算法的路径的时候，其空间复杂度可以缩减到`O(2N) -> O(2N)`，这里的N是其中1条序列的长度。`traj-dist-rs`会默认使用这种算法记录动态规划算法的中间值，实现内存使用的下降。
...
2. 完整的性能测试记录：https://github.com/Davidham3/traj-dist-rs/blob/v0.1.0-alpha.3/docs/performance.md。
